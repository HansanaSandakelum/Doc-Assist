...
import {getLogout, setRedirectPath, getLoginSuccess} from "../../../redux/actions/actions";
import { CircularProgress } from "@mui/material";

const numInputs = 6
const LoginVerification = (props: any) => {
    ...
    const [disabledTime, setDisabledTime] = useState(startTime + enableGetOtpTime)
    const [countDown, setCountDown] = useState(disabledTime - new Date().getTime());
    ...
    const renderer = ({days, hours, minutes, seconds, onCompleted}: any) => {
        if (onCompleted) {
            return <Typography variant="h3" color='error'>00 : 00</Typography>;
        } else {
            return <Typography variant="h3"
                               color='success'>{minutes} : {seconds}</Typography>;
        }
    };
    ...
    const verify = (values: any) => {
        ...
        AuthService?.verifyLogin(formData).then(
            response => {
    ...
    const otpGet = () => {
        setLoadingOtp(true)
        AuthService?.resendLoginOtp().then(
            response => {
                if(response?.isSuccess){
                    setLoadingOtp(false)
                                    ...
                                    <Grid container alignItems="center" justifyContent="center">
                                        ...
                                        <Grid item xs={12} marginTop={2} textAlign="center">
                                            <Button type="button" color="info" variant="contained"
                                                    disabled={otp.trim() === ''} onClick={clearOtp}
                                                    sx={{m: 2}}>
                                                Clear
                                            </Button>
                                            <Button color="info" type="button" variant="contained"
                                                    disabled={loadingOtp || countDown > 0}
                                                    startIcon={loadingOtp ? <CircularProgress color="inherit" sx={{mr:1}} size={20} /> : <CachedIcon/>}
                                                    onClick={otpGet}>
                                                Get OTP
                                            </Button>
                                        </Grid>
                                        ...
                                        color="primary"
                                        onClick={() => verify(otp)}>VERIFY</Button>
                                        <Grid item>
